...
class HolidayService:
    ...
    async def get_holidays(self, guild):
        ...
        try:
            ...
            logger.error(f"Failed to retrieve holidays for guild {guild.name}: {str(e)}")
            return {}
    ...
    async def remove_all_except_current_holiday_role(self, guild: discord.Guild, current_holiday_name: str):
        ...
        logger.info(f"Starting to remove all holiday roles except for '{current_holiday_name}' in guild '{guild.name}'.")
        ...
        try:
            ...
            logger.error(f"Failed to remove holiday roles due to an error: {str(e)}")
    ...
    async def apply_holiday_role(self, guild, holiday_name, dry_run):
        ...
        try:
            async with self.config.guild(guild).holidays() as holidays:
                ...
                original_name = next((name for name in holidays if name.lower() == normalized_holiday_name), holiday_name)
                holiday_details = normalized_holidays[normalized_holiday_name]
                logger.debug(f"Holiday details retrieved: {holiday_details}")

                role = await self.role_manager.create_or_update_role(guild, original_name, holiday_details['color'], holiday_details['date'], holiday_details.get('image'))
            if role:
            ...
            return False, f"Failed to apply holiday role due to an HTTP error: {str(e)}"