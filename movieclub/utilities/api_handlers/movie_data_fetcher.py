...
def movie_data_to_discord_format(movie_data: Dict[str, Any]) -> Dict[str, Any]:
    try:
        ...
        fields = [
            {'name': 'Details', 'value': f"{', '.join(movie_data['genre'][:2])} · {movie_data['runtime']} mins", 'inline': True},
            {'name': 'Rating', 'value': f"★ {movie_data['rating']} · {convert_to_presentable_count(movie_data['number_of_reviewers'])} fans", 'inline': True},
