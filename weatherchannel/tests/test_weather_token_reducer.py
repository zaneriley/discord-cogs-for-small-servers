...
def test_city_data_handling(all_cities_fixture):
    ...
    for city in all_cities_fixture:
...
class TestWeatherTokenReducer(unittest.TestCase):
    ...
    def test_reduce_weather_dict(self):
        ...
        assert "periods" in result["Houston"]

        period = result["Houston"]["periods"][0]
        assert period["name"] == "Tonight"
        assert period["temp"] == 65
        assert period["unit"] == "F"
        assert period["forecast"] == "Cloudy"
        assert period["precip"] == 40
    ...
    def test_daily_summary(self):
        ...
        assert "Monday: 75°" in result
        assert "Tuesday: 80°" in result
        assert "Monday Night" not in result  # Night periods should be excluded
    ...
    def test_llm_prompt_format(self):
        ...
        assert "Miami:" in result_with_alerts
        assert "Today: 90°F" in result_with_alerts
        assert "ALERTS:" in result_with_alerts
        assert "Heat Advisory" in result_with_alerts

        # Test without alerts
        result_without_alerts = self.reducer.format_for_llm_prompt(weather_data, include_alerts=False)
        assert "Miami:" in result_without_alerts
        assert "Today: 90°F" in result_without_alerts
        assert "ALERTS:" not in result_without_alerts
    ...
    def test_tokyo_data_handling(self):
        ...
        if "Tokyo" in self.test_data:
            ...
            assert "" not in result  # Should not have encoding issues
